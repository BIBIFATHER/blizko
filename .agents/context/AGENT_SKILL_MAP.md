...
## 8A. Full Codebase Review / Audit

**Когда:**
- полный анализ кода
- codebase audit
- полный code review
- health + architecture + security review всей системы
...
**Support agents:**
- `qa-release`
- `trust-safety` if auth / payments / moderation / personal data / admin paths matter
- `joe` if conversion-critical UI or onboarding flows are materially involved
...
- `security-review`
- `webapp-testing`
- `react-useeffect` if React behavior / effects are involved

**Правило:**
- это review-задача, а не generic analytics
- findings first
- severity ordered
- file references mandatory
- static analysis alone is insufficient
- явно перечислить: что проверено, что не проверено
