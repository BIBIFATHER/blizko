...
| RISK-009 | Supabase RLS/grant gaps: `chat_participants` INSERT lets any authenticated user join any thread (read others' chat); `support_messages` INSERT allows sender spoofing; broad `GRANT ALL` to anon/authenticated on PD tables. | P1 | Open | Codex review 2026-06-18 vs `00000000000000_remote_schema.sql` (chat_participants l.751, chat_messages SELECT l.627, support_messages l.669, GRANT ALL l.856/926, default privileges l.958); hardening plan PR #44; Linear BLI-124. Synthetic-only now (only owner accounts) — not actively exploited. This is the sharpest DB/RLS vuln, NOT the sharpest product risk (that is RISK-001/BLI-121). | Tighten participant/message INSERT policies + least-privilege grants via DB protocol (`supabase db push`) + owner approval before real users. Plan: `docs/architecture/supabase-security-hardening-plan.md`. |
