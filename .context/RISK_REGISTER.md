...
| RISK-006 | RU personal-data data plane is not yet proven for real pilot users. | P1 | In Progress | 2026-06-30: **direction changed to Yandex Cloud** — YC approved a 10,000 ₽ / 2-month grant; supersedes the Timeweb PoC. Plan: `docs/architecture/ru-core-yandex-cloud-poc.md`. Grant = synthetic PoC budget only (single VM, self-host Supabase + PG 17.x, Object Storage; managed PG / HA / ФСТЭК-attested segment NOT covered). Phase 3 PoC not started (gated). | Run T0-T16 synthetic PoC on Yandex Cloud (self-host Supabase) after BLI-121 + legal drafts; synthetic may start on YC access. **Conditional Go synthetic / No-Go real users** until BLI-121, BLI-124+BLI-134, legal/privacy/RKN (BLI-123), and ИБ УЗ assessment (children/documents → high УЗ + ФСТЭК) are done. |
