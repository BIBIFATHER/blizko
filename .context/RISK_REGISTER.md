...
| RISK-009 | `chat_participants` RLS may allow self-joining arbitrary threads and then reading other users' chat messages. | P1 | Open | 2026-06-18 hardening plan F8 identifies INSERT policy that only checks `auth.uid() = user_id`; direct participant upsert exists in frontend. | Fix insert/upsert policy before real users; require thread membership / support helper / service_role path and re-run advisors. |
| RISK-009 | Supabase RLS/grant gaps: `chat_participants` INSERT lets any authenticated user join any thread (read others' chat); `support_messages` INSERT allows sender spoofing; broad `GRANT ALL` to anon/authenticated on PD tables. | P1 | Open | Codex review 2026-06-18 vs `00000000000000_remote_schema.sql` (chat_participants l.751, support_messages l.669); hardening plan PR #44. Synthetic-only now (only owner accounts) — not actively exploited. | Tighten participant/message INSERT policies + least-privilege grants via DB protocol (`supabase db push`) + owner approval before real users. Plan: `docs/architecture/supabase-security-hardening-plan.md`. |
