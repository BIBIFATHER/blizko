...
const NannyForm = lazy(() =>
  import('@/components/NannyForm').then((module) => ({ default: module.NannyForm })),
);
const SuccessScreen = lazy(() =>
  import('@/components/SuccessScreen').then((module) => ({ default: module.SuccessScreen })),
);
const MatchResultsScreen = lazy(() =>
  import('@/components/MatchResultsScreen').then((module) => ({
    default: module.MatchResultsScreen,
  })),
);
...
const NannyLandingPage = lazy(() =>
  import('@/components/NannyLandingPage').then((module) => ({ default: module.NannyLandingPage })),
);
const NannyPublicProfile = lazy(() =>
  import('@/components/nanny/NannyPublicProfile').then((module) => ({
    default: module.NannyPublicProfile,
  })),
);
...
const AboutPage = lazy(() =>
  import('@/components/legal/LegalPages').then((module) => ({ default: module.AboutPage })),
);
const SafeDealPage = lazy(() =>
  import('@/components/legal/LegalPages').then((module) => ({ default: module.SafeDealPage })),
);
const PrivacyPage = lazy(() =>
  import('@/components/legal/LegalPages').then((module) => ({ default: module.PrivacyPage })),
);
const SupportChat = lazy(() =>
  import('@/components/SupportChat').then((module) => ({ default: module.SupportChat })),
);
...
const RoleDashboard = lazy(() =>
  import('@/components/dashboard/RoleDashboard').then((module) => ({
...
export default function App() {
  ...
  const devMock = (() => {
    ...
    const mockEmail =
      params.get('mockEmail') || (mockAdmin ? adminEmails[0] || 'admin@local.dev' : undefined);
  ...
  const { isShareModalOpen, setShareModalOpen, handleShare } = useShareActions({ lang });
  const { deferredPrompt, showInstallModal, setShowInstallModal, platform, handleInstallClick } =
    usePwaInstall();
  const [shouldLoadSupportChat, setShouldLoadSupportChat] = useState(
    () => location.pathname !== '/',
  );
  ...
  useEffect(() => {
    const openAuthFromEvent = (event: Event) => {
      const source =
        event instanceof CustomEvent ? event.detail?.source || 'custom_event' : 'custom_event';
  ...
  const isAdmin =
    !!(effectiveUser?.email && adminEmails.includes(String(effectiveUser.email).toLowerCase())) ||
    !!devMock?.isAdmin;
  ...
  const pageSeo = (() => {
    if (currentPath === '/find-nanny') {
      return {
        title:
          lang === 'ru'
            ? 'Найти няню в Москве | Заявка в Blizko'
            : 'Find a nanny in Moscow | Blizko request',
        description:
          lang === 'ru'
            ? 'Оставьте заявку и получите подборку из 2–3 проверенных нянь с понятными причинами выбора.'
            : 'Submit your request and receive a small shortlist with moderated profiles and clear reasons for each choice.',
    ...
    if (currentPath === '/become-nanny') {
      return {
        ...
        description:
          lang === 'ru'
            ? 'Заполните анкету, пройдите проверку и решите, когда активировать профиль для получения заказов.'
            : 'Complete your profile, pass review, and decide when to activate it to receive requests.',
    ...
    if (currentPath === '/for-nannies') {
      return {
        ...
        description:
          lang === 'ru'
            ? 'Blizko помогает няням пройти путь от анкеты и проверки до более понятных запросов от семей.'
            : 'Blizko helps nannies move from profile review to clearer family requests.',
    ...
    return {
      title:
        lang === 'ru'
          ? 'Blizko — спокойный подбор няни для семьи'
          : 'Blizko — calmer nanny search for families',
      description:
        lang === 'ru'
          ? 'Blizko помогает пройти путь от тревожного поиска к подборке из 2–3 проверенных нянь с понятными причинами выбора.'
          : 'Blizko helps families move from anxious searching to a shortlist of 2-3 candidates with profile moderation and clear reasons for each choice.',
  ...
  return (
    <div className="app-scaffold min-h-screen text-stone-700 font-sans selection:bg-amber-100 flex flex-col pb-safe">
      ...
      <main
        className={`app-main-frame flex-1 relative${currentPath === '/find-nanny' || currentPath === '/become-nanny' || currentPath === '/match-results' ? ' flow-frame' : ''}`}
      >
        <Suspense fallback={<RouteFallback />}>
          <Routes>
            ...
            <Route
              path="/find-nanny"
              element={<ParentForm onSubmit={handleParentSubmit} lang={lang} />}
            />
            <Route
              path="/become-nanny"
              element={<NannyForm onSubmit={handleNannySubmit} lang={lang} />}
            />
      ...
      {isAuthOpen && (
        <Suspense fallback={<RouteFallback />}>
          <AuthModal onClose={() => setAuthOpen(false)} onLogin={handleLogin} lang={lang} />
        </Suspense>
      )}
      ...
      {isShareModalOpen && (
        <Suspense fallback={<RouteFallback />}>
          <ShareModal onClose={() => setShareModalOpen(false)} lang={lang} />
        </Suspense>
      )}
