...
## Решения
- Таблицы **matches / nanny_verifications / nanny_risk_flags** — **нужны**.
- Таблица **payments** — позже, когда появятся реальные транзакции.
- Схема хранится в **SQL‑миграциях** (не только ручные правки в Supabase).

## Админ‑доступ
- Единственный админ — Антон.
- Админ‑доступ только через service role (сервер), без клиентских ролей.
