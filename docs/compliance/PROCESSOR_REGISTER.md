...
| Yandex Cloud (RU-core: Compute, Object Storage, Lockbox/KMS, Audit Trails) | planned RU data-plane host (self-hosted Supabase + PG 17.x, private S3) | would host profiles, documents, photos, chat, transactions — **none yet; synthetic-only PoC** | RU region (152-FZ Art.18.5); ЦОД evidence to capture at T0 | DPA / поручение на обработку NOT yet signed | Backup/restore/deletion to prove in T6/T10/T16 | `Planned / not active`: 10,000 ₽/2-mo grant for synthetic PoC only; receives NO real personal data until BLI-121 + legal/RKN + ИБ УЗ (children/documents → high УЗ + ФСТЭК) + T0-T16 evidence. Plan: `docs/architecture/ru-core-yandex-cloud-poc.md` |
...
| Google Fonts (`fonts.googleapis.com`/`fonts.gstatic.com`) | web font CDN | visitor IP, User-Agent (per page load) | Google infrastructure (cross-border) | Google Fonts terms; no DPA | Provider-side | `Removed` (BLI-122): fonts self-hosted via `@fontsource` (bundled by Vite, served same-origin); Google `<link>` + `fonts.*` CSP allowances removed. No more IP egress to Google |
| Unsplash (`images.unsplash.com`) | image CDN | visitor IP, User-Agent (per image load) | Unsplash/Imgix infrastructure (cross-border) | Unsplash terms; no DPA | Provider-side | `Removed` (BLI-122): hero + nanny-placeholder + admin-preview images self-hosted under `public/assets/`; `images.unsplash.com` dropped from CSP. No IP egress to Unsplash |
