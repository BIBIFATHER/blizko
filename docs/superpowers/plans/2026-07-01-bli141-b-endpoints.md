# BLI-141 План B — Server-authoritative endpoints Implementation Plan (rev 9)
...
- **C7 — Status optimistic concurrency c `expected_status` (Codex round2 P1, дизайн §174).** Тело status-эндпойнта = `{ booking_id, expected_status, to_status }`. Нормативный порядок (код=текст=тесты, Codex round5): **auth-401 → presence-400 → load booking-404 → role-403** (actor вне матрицы) **→ matrix** `expected_status ∈ from(to_status)`-**400 → optimistic** `UPDATE … WHERE id=? AND status=$expected_status`; 0 строк → **409** (stale: реальное состояние ≠ ожидаемого). Так `403` (нет права/переход невозможен) и `409` (устаревшее чтение) разделены; клиент не применит действие из устаревшего UI к более новому состоянию. Тест: unauthenticated + malformed → **401** (auth раньше presence).
...
**Interfaces:**
- Produces: `POST /api/bookings?op=status` тело `{ booking_id, expected_status, to_status }` (C7). verifyBearerUser (участник) ИЛИ verifyBearerAdmin. Порядок (совпадает со сниппетом): **auth-401 → presence-400** → load booking-404 → role-403 → matrix `expected_status ∈ from(to_status)`-400 → optimistic `UPDATE … WHERE id=? AND status=$expected_status`-иначе-409(stale). Ответы `200 {booking}` / `401` / `400` (presence ИЛИ matrix expected∉from) / `404` / `403` (роль) / `409` (stale).
...
async function updateStatus(req: VercelRequest, res: VercelResponse) {
  ...
  // Нормативный порядок (C7): auth(401, выше) → presence(400) → load booking(404) → role(403)
...
// Timeout с очисткой таймера; при зависании statement_timeout/lock_timeout (pool) отменит
// сами PG-операции → pA/pB отклонятся, клиенты вернутся в pool, afterAll pool.end() не повиснет.
let timer: NodeJS.Timeout;
const guard = new Promise((_, rej) => { timer = setTimeout(() => rej(new Error('create race deadlock/timeout')), 10000); });
try { await Promise.race([Promise.all([pA, pB]), guard]); }
