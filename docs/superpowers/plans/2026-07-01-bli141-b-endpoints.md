...
- **C10 — Единый lock order (Codex round6/7 P1, дизайн §3.1/§6).** Против cross-process deadlock create↔lifecycle: строго по приоритету ТАБЛИЦ **`parents` → `nannies` → `account_deletions` → `bookings`**; при нескольких строках одной таблицы — по её ключу asc (**`parents.id`, `nannies.id`, `account_deletions.user_id`, `bookings.id`** — у `account_deletions` ключ `user_id`, НЕ `id`). НЕ сортировать между таблицами. Create (Task 1) лочит `parents(request_id)` → `nannies(nanny_entity_id)` — канон. **Lifecycle (§6) обязан следовать тому же: сперва lock `parents`/`nannies` пары, ПОТОМ upsert `account_deletions`** (§6 перенумерован — deletion-флаг больше НЕ шаг 1). Concurrency-тест (Task 7) проверяет create↔create.
...
- State-machine actor (owner-решение): `confirmed→active` и `active→completed` двигает **няня** (auth-uid=nanny_id) или куратор. Матрица §5. Единый контракт кодов — **C7** (не дублировать семантику здесь): роль не разрешена → 403; `(expected_status→to_status)` не строка матрицы → 400; реальный статус ≠ expected → 409 (stale).
- **C7 — Status optimistic concurrency c `expected_status` (Codex round2/8 P1, дизайн §174/§5).** Тело = `{ booking_id, expected_status, to_status }`. Нормативный порядок (код=текст=тесты): **auth-401 → presence+matrix-400** (поля есть, `to_status∈TRANSITIONS`, `expected_status∈rule.from`) **→ load booking-404 → role-403** (роль ∈ `rule.roles`) **→ optimistic** `UPDATE … WHERE id=? AND status=$expected_status`; 0 строк → **409** (stale). **Matrix-membership проверяется ДО role** (Codex round8): невозможная пара (напр. parent `pending→active`) всегда `400`, не маскируется `403`. Три исхода разделены: `400` невозможный переход, `403` нет права, `409` устаревшее чтение. Тесты: unauthenticated+malformed → `401` (auth раньше); **non-admin invalid-pair → `400`** (matrix раньше role).
...
- `api/bookings.ts` — `computeFingerprint`; create-порядок: валидация → **idempotency-first** → lock/load (FOR UPDATE) → derive → eligibility → deletion-guard → pair-cardinality → INSERT; status actor state-machine; admin GET. Одна функция-хендлер, роутинг по `req.method` + `req.query.op`; default-closed gate (C9).
...
### Task 1: fingerprint + create endpoint (валидация → idempotency-first → lock/load → derive → eligibility → deletion-guard → pair → insert)
...
**Interfaces:**
- Produces: `POST /api/bookings?op=status` тело `{ booking_id, expected_status, to_status }` (C7). verifyBearerUser (участник) ИЛИ verifyBearerAdmin. Порядок (совпадает со сниппетом): **auth-401 → presence+matrix-400** (`expected_status∈from(to_status)`) → load booking-404 → role-403 → optimistic `UPDATE … WHERE id=? AND status=$expected_status`-иначе-409(stale). **Matrix ДО role** (round8). Ответы `200 {booking}` / `401` / `400` (presence ИЛИ невозможная пара) / `404` / `403` (роль) / `409` (stale).

- [ ] **Step 1: Write failing tests**

```ts
describe('POST /api/bookings?op=status', () => {
  // Codex round6 P1: сбрасывать connect ПЕРЕД каждым status-тестом, иначе
  // накопленные вызовы из предыдущих тестов провалят connect.not.toHaveBeenCalled() в 401-тесте.
  beforeEach(() => connect.mockReset());
  ...
  it('unauthenticated + malformed body → 401 (auth раньше presence)', async () => {
    const { verifyBearerAdmin, verifyBearerUser } = await import('./_auth.js');
    (verifyBearerAdmin as unknown as { mockResolvedValueOnce: (v: unknown) => void }).mockResolvedValueOnce(null);
    (verifyBearerUser as unknown as { mockResolvedValueOnce: (v: unknown) => void }).mockResolvedValueOnce(null);
    const res = createMockResponse();
    await handler(statusReq({}), res); // тело пустое (malformed) + нет auth → 401, не 400
  ...
  it('non-admin invalid-pair → 400 (matrix ДО role; parent pending→active НЕ маскируется 403)', async () => {
    ...
    (verifyBearerAdmin as unknown as { mockResolvedValueOnce: (v: unknown) => void }).mockResolvedValueOnce(null);
    (verifyBearerUser as unknown as { mockResolvedValueOnce: (v: unknown) => void })
      .mockResolvedValueOnce({ id: 'parent-uid', email: 'p@x' }); // participant, но роль тут неважна
    const res = createMockResponse();
    // to_status='active' допускает from только 'confirmed'; expected 'pending' — не строка матрицы → 400.
    // Проверяется в presence-тире ДО load/role → connect не вызван, parent НЕ получает 403.
...
async function updateStatus(req: VercelRequest, res: VercelResponse) {
  ...
  // Нормативный порядок (C7, Codex round8): auth(401,выше) → presence+matrix(400)
  //   → load booking(404) → role(403) → optimistic update(409). Matrix-membership
  //   (expected_status ∈ rule.from) проверяем ЗДЕСЬ, ДО role — иначе невозможная пара
  //   (напр. parent pending→active) маскируется 403 вместо 400. Код=текст=тесты.
  ...
  try {
    ...
    // matrix (expected∈rule.from) уже проверен в presence-тире (400 до role). Здесь — optimistic:
    // применяем только если реальный статус == expected_status клиента.
...
**DI зафиксирован однозначно:** `_cors`/`_rate-limit` мокнуты пропускающими, `_auth` мокнут (admin/участник). `_db` **мокнут так, чтобы `getDbPool()` возвращал ЕДИНЫЙ тестовый `pg.Pool` с `max>=3`**, подключённый к `INTEGRATION_PG_URL` (НЕ app-singleton `max:1` из `api/_db.ts:46` — он сериализует до row-lock). **`max>=3` обязателен для case 7 (Codex round5 P1):** нужны ОДНОВРЕМЕННО три checked-out клиента — lock-holder `L` + handler `A` + handler `B`; при `max:2` один handler ждал бы pool-коннект (а не row-lock) → ложный pass. SQL реальный. Тестовый pool создаётся с **уникальным per-run** `application_name` — `const APP_NAME = 'bli141_ctest_' + process.pid + '_' + Date.now()` (через `?application_name=` в conn-string или опции pool) — чтобы observer в case 7 фильтровал `pg_stat_activity` строго по СВОИМ backend'ам, не по параллельным BLI-141 пулам с тем же именем. `beforeAll`: создать тестовый pool (`application_name=APP_NAME`, `statement_timeout=8000`, `lock_timeout=8000` — чтобы зависший create отменялся PG, а не висел вечно) + fixture `parents(id,payload,user_id)` `payload='{"status":"new"}'`, `nannies(...)`. `afterAll`: DELETE fixture + `pool.end()`.
...
```ts
// pool.max>=3: L + A + B одновременно checked out (иначе handler ждал бы pool-коннект, не row-lock).
...
try {
  ...
  pA = handler(makeCreate(reqId, nannyId, uuidA), resA);                   // блокируются на lock parents
  pB = handler(makeCreate(reqId, nannyId, uuidB), resB);
  // Observer через L.query (L уже checked-out; read-only SELECT не берёт новый client → 3 клиента при max>=3):
  await waitFor(async () => {
    const { rows } = await L.query(
      `SELECT count(*)::int AS n FROM pg_stat_activity
       WHERE application_name=$1 AND pid <> pg_backend_pid()  -- только СВОИ backend'ы (per-run), не L
         AND wait_event_type='Lock' AND state='active' AND query ILIKE '%parents%FOR UPDATE%'`,
      [APP_NAME]);
    return rows[0].n === 2;              // A и B оба взяли DB-client и ждут именно row-lock
  }, { timeout: 5000 });
  await L.query('ROLLBACK');             // отпустить lock → A,B продолжают ПО ОЧЕРЕДИ
  const guard = new Promise((_, rej) => { timer = setTimeout(() => rej(new Error('create race deadlock/timeout')), 10000); });
  ...
  // L всегда закрывается, даже если waitFor/guard бросили → afterAll pool.end() не виснет.
  clearTimeout(timer);
  try { await L.query('ROLLBACK'); } catch { /* already rolled back — no active txn */ }
  L.release();
  // на аварийном пути statement_timeout/lock_timeout (pool) отменит зависшие create →
  // pA/pB отклонятся, их клиенты вернутся в pool; дождаться расчёта, игнорируя ошибки:
...
- План C: `updateBookingStatus` клиента → status endpoint (снятие всех status-writer'ов — `AdminPanel.tsx:338`/`BookingsTab.tsx:152`/`AdminPage.tsx:348` — **входит в прод-cutover-бандл B**, C0); `delete-account` lifecycle + `account_deletions` reconciler (наполняет таблицу → включает deletion-guard); удаление local **readers**/pending-merge (§7). **Lifecycle обязан соблюдать единый lock order C10** (`parents`→`nannies`→`account_deletions`→`bookings`) — иначе deadlock с create. **Durability idempotency — bounded (Опция B, Codex round3 P1):** lifecycle §6 удаляет unpaid-pending (шаг 2) / обезличивает терминальные (шаг 4); replay гарантирован лишь пока строка жива, после delete — нет. Никакого «не DELETE-ит» инварианта (противоречил бы §6/152-ФЗ). Отдельного durable-хранилища нет.
