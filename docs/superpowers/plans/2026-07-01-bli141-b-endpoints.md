# BLI-141 План B — Server-authoritative endpoints Implementation Plan (rev 4)
...
7. **Concurrency (Codex round2 P2 #2 + round3 P2):** два create одной пары (разные ключи) параллельно через тестовый pool `max>=2` (Step 1) → ровно один 201, второй 409. Барьер удерживает **оба соединения до попытки захвата `parents`/`nannies` row-lock** (напр. первый `handler` держит транзакцию открытой на checkpoint, второй стартует; затем отпустить) — иначе pool-сериализация замаскирует `FOR UPDATE`. Assert один 201/один 409 + таймаут на дедлок. DI — из Step 1 (единый pool max≥2, не app-singleton).
