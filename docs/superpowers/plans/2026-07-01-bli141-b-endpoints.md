# BLI-141 План B — Server-authoritative endpoints Implementation Plan (rev 8)
...
- **C10 — Единый lock order (Codex round6 P1, дизайн §3.1).** Против cross-process deadlock create↔lifecycle: строго по приоритету таблиц **`parents` → `nannies` → `account_deletions` → `bookings`**, внутри таблицы по `id` asc; НЕ id-sort между таблицами. Create в Task 1 уже лочит `parents(request_id)` затем `nannies(nanny_entity_id)` — канон. Lifecycle (План C) обязан следовать тому же порядку; concurrency-тест (Task 7) косвенно проверяет create↔create.
...
**DI зафиксирован однозначно:** `_cors`/`_rate-limit` мокнуты пропускающими, `_auth` мокнут (admin/участник). `_db` **мокнут так, чтобы `getDbPool()` возвращал ЕДИНЫЙ тестовый `pg.Pool` с `max>=3`**, подключённый к `INTEGRATION_PG_URL` (НЕ app-singleton `max:1` из `api/_db.ts:46` — он сериализует до row-lock). **`max>=3` обязателен для case 7 (Codex round5 P1):** нужны ОДНОВРЕМЕННО три checked-out клиента — lock-holder `L` + handler `A` + handler `B`; при `max:2` один handler ждал бы pool-коннект (а не row-lock) → ложный pass. SQL реальный. Тестовый pool создаётся с **`application_name='bli141_ctest'`** (через `?application_name=` в conn-string или `application_name` в опциях) — чтобы observer в case 7 фильтровал `pg_stat_activity` только по своим backend'ам, не по параллельным тестам. `beforeAll`: создать тестовый pool + fixture `parents(id,payload,user_id)` `payload='{"status":"new"}'`, `nannies(...)`. `afterAll`: DELETE fixture + `pool.end()`.
...
await waitFor(async () => {
  const { rows } = await L.query(
    `SELECT count(*)::int AS n FROM pg_stat_activity
     WHERE application_name='bli141_ctest' AND pid <> pg_backend_pid()  -- только свои backend'ы, не L
...
// Реальный timeout вокруг Promise.all (иначе дедлок повесил бы тест бесконечно):
await Promise.race([
  Promise.all([pA, pB]),
  new Promise((_, rej) => setTimeout(() => rej(new Error('create race deadlock/timeout')), 10000)),
]);
