...
- **C3 — Idempotency-key = клиент-UUID, стабилен на operation-attempt (Codex round1 #5 + round2: дизайн §147).** Ключ — **глобально уникальный UUID** (`crypto.randomUUID()`), сгенерированный **один раз на попытку операции** и удерживаемый стабильным на **транспортных ретраях этой попытки** (сеть отвалилась → повтор с ТЕМ ЖЕ ключом → 200 replay). **Новое пользовательское действие = новый UUID.** НЕ детерминированный hash от `(request_id,nanny_entity_id,date)` — тот превращал бы idempotency-token в бизнес-дедуп: повторное законное назначение той же пары после `cancelled`/`completed` навсегда replay-ило бы старую строку, а смена `amount` давала вечный 409 (Codex round2 P1). Бизнес-дедуп «одна активная бронь на пару» — отдельная забота **pair-cardinality** (C1), НЕ ключа. Fingerprint (server-side) отделяет legit-replay (тот же ключ+payload → 200) от reuse-конфликта (тот же ключ, иной payload → 409).
  - Стабильность на attempt реализуется в клиенте: UUID генерится один раз в `handleAssign` и переиспользуется, если сам вызов `createBooking` делает внутренние транспортные ретраи. Двойной клик пользователя = новая попытка = новый UUID → упрётся в pair-cardinality (409, если активная пара уже есть), дубля не будет.
...
- **C8 — Строгая валидация входа (Codex round2 P2).** ДО транзакции: `date` строго `YYYY-MM-DD` + реальная календарная дата (отсекает `2026-02-31`); `idempotency_key` — UUID (regex); `request_id`/`nanny_entity_id` — лимит длины (≤128); `amount` — канон decimal `^\d{1,9}(\.\d{1,2})?$` (пока TEXT; numeric — План E). Любой невалид → **400** ДО парсинга (`new Date` не бросает внутри транзакции). Тесты malformed: битая дата, не-UUID ключ, overflow-amount.
...
- `src/components/admin/AdminCuratorTab.tsx` — `handleAssign` генерит `idempotency_key = crypto.randomUUID()` **один раз на попытку** (C3), шлёт `request_id`+`nanny_entity_id`+`idempotency_key`; истинный результат.
...
COMMIT → 201
```
**Durability replay (Codex round2 P1 #4):** idempotency evidence — сама строка `bookings`. Lifecycle (План C/§6/§7) booking-строки **обезличивает** (`parent_erased_at`/`nanny_erased_at`, SET NULL), НЕ удаляет → `idempotency_key`/`idempotency_fingerprint` переживают удаление аккаунта, replay остаётся валиден. Инвариант «lifecycle не DELETE-ит booking-строки с idempotency_key» фиксируется как контракт Плана C (записать в §7). Отдельная idempotency-таблица не вводится (YAGNI, пока строка durable).
...
const AMOUNT_RE = /^\d{1,9}(\.\d{1,2})?$/;             // канон decimal (amount пока TEXT; numeric — План E)
...
// date уже провалидирован (YYYY-MM-DD) → без throw-парсинга.
...
async function createBooking(req: VercelRequest, res: VercelResponse) {
  ...
  if (amount != null && !AMOUNT_RE.test(amount)) return json(res, 400, { error: 'amount must be decimal' });
...
describe('POST /api/bookings?op=status', () => {
  ...
  it('expected_status not a valid source for to_status → 400', async () => {
    const { verifyBearerAdmin } = await import('./_auth.js');
    (verifyBearerAdmin as unknown as { mockResolvedValueOnce: (v: unknown) => void })
      .mockResolvedValueOnce({ id: 'admin-1', email: 'admin@example.com' });
    const res = createMockResponse();
    // to_status='active' допускает from только 'confirmed'; expected 'pending' невозможен → 400
...
async function updateStatus(req: VercelRequest, res: VercelResponse) {
  ...
  // expected_status обязан быть допустимым from для to_status (иначе клиент просит невозможный переход).
  if (!rule.from.includes(expected_status)) {
    return json(res, 400, { error: 'expected_status is not a valid source for to_status' });
  }
  ...
  try {
    ...
    // (2) optimistic: применяем только если реальный статус == expected_status клиента.
...
**Interfaces:**
- Produces: `createBooking({ request_id, nanny_entity_id, idempotency_key, date, amount? })` — authed POST `/api/bookings?op=create`; возвращает `Booking` или **бросает** (нет ложного local success). `idempotency_key` — UUID, генерится **вызывающим** (Task 6, один раз на попытку, C3); сервис его НЕ генерит и НЕ выводит из полей.
...
```tsx
const idempotency_key = crypto.randomUUID(); // один UUID на попытку (C3)
await createBooking({
...
```
Убрать fallback `?? id` полностью. `reportSuccess` только после await; `catch` → `reportError` (есть). Ключ — свежий UUID на каждую попытку назначения (C3): защита от дублей — серверная `pair-cardinality` (двойной клик при активной паре → 409 → `reportError`, дубля нет). Ключ стабилен только на транспортных ретраях внутри одного вызова (idempotency-replay 200).
...
7. **Concurrency (Codex round2 P2 #2):** два create одной пары (разные ключи) параллельно → ровно один 201, второй 409. **КРИТИЧНО:** общий `getDbPool()` — `max:1` (`api/_db.ts:46`) сериализует запросы в пуле ДО row-lock, тест не докажет `FOR UPDATE`. Тест обязан открыть **два независимых `pg.Pool` (или два `client`) с `max>=2`**, стартовать оба create через барьер (оба дошли до `BEGIN`), затем отпустить; assert один 201/один 409 + таймаут на дедлок. Прогонять хендлер с инъекцией разных pool'ов (параметризовать `getDbPool` в тесте), не общий singleton.
...
- `api/bookings.ts`: create (валидация C8 → **idempotency-first** до mutable-guards → провенанс → eligibility → deletion-guard → pair-cardinality → 23505-reread), status (actor state-machine + `expected_status` optimistic, 403 vs 409 разделены), admin GET. Actor-scoped rate-limit (C5).
- Клиентский `createBooking`/`handleAssign` шлют entity-id + **UUID** idempotency-key (сгенерён один раз на попытку, не hash); сервер выводит auth-uid; нет `?? id` fallback; нет ложного local success.
...
- План C: `updateBookingStatus` клиента → status endpoint (снятие всех status-writer'ов — `AdminPanel.tsx:338`/`BookingsTab.tsx:152`/`AdminPage.tsx:348` — **входит в прод-cutover-бандл B**, C0); `delete-account` lifecycle + `account_deletions` reconciler (наполняет таблицу → включает deletion-guard); удаление local **readers**/pending-merge (§7). **Контракт durability idempotency (Codex round2 P1 #4):** lifecycle обезличивает booking-строки (SET NULL/erased_at), НЕ DELETE-ит строки с `idempotency_key` → записать в §7 как инвариант.
