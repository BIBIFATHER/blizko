# BLI-141 План B — Server-authoritative endpoints Implementation Plan (rev 2)
...
- Идентичность: клиент шлёт `request_id` (=`parents.id`, TEXT) + `nanny_entity_id` (=`nannies.id`, TEXT) + `idempotency_key` (детерминированный, см. C3); сервер выводит `parent_id=parents.user_id`, `nanny_id=nannies.user_id`. **Клиент НЕ шлёт auth-uid; нет fallback `?? id`.** Схема (`00000000000000_remote_schema.sql`): `parents(id text, payload jsonb, user_id uuid NOT NULL default auth.uid())`, `nannies` симметрично.
...
- **C0 — Плана B НЕ деплоится в прод отдельно (rollout-секвенс, Codex P1 #2).** До Плана E политика `bookings_participant` (ALL) и `GRANT ALL … authenticated` на `bookings` сохраняются → клиент физически может писать в `bookings` напрямую через PostgREST в обход всех инвариантов эндпойнта (провенанс, eligibility, idempotency, cardinality). Поэтому:
  - Плана B код (endpoint + клиентский switch) лендится и тестируется на **preview/staging**, а прод-включение стоит за фиче-флагом.
  - **Прод-cutover бандлит одним контролируемым релизом (§10):** (a) deploy `api/bookings.ts`; (b) удаление клиентских прямых writer'ов; (c) Плана E lockdown — RLS-split `bookings` + REVOKE write у `authenticated` + `bookings_active_pair_uq`; (d) lifecycle C — `account_deletions` наполняется (иначе deletion-guard спит, см. C4).
...
- **C3 — Idempotency-key стабилен на интент (Codex P1 #5).** Клиент НЕ шлёт `randomUUID()` (иначе ретрай той же кнопки = новый ключ = дубль/пара-409). Ключ детерминирован от интента: `idempotency_key = 'bk:v1:' + sha256(request_id ⊳ nanny_entity_id ⊳ date)` (⊳ = `\x1f`). Ретрай того же назначения → тот же ключ → **200 replay**. Тот же ключ с иным `amount` → **409** (реальный конфликт полезной нагрузки, ловится через fingerprint).
...
- `api/bookings.ts` — `computeFingerprint`, provenance derivation, eligibility, deletion-guard, idempotency-first, pair-cardinality, INSERT; status actor state-machine; admin GET. Одна функция-хендлер, роутинг по `req.method` + `req.query.op`.
...
- `src/services/booking.ts` — заменить `createBooking`/(позже `updateBookingStatus`) на серверные вызовы; удалить прямой `supabase.from('bookings')` write в create-path + local-first pending. Local **readers**/pending-merge — План C (§7), здесь только create-writer.
- `src/services/bookingKey.ts` — детерминированный `computeBookingKey(request_id, nanny_entity_id, date)` (C3), общий клиент/тест.
- `src/components/admin/AdminCuratorTab.tsx` — `handleAssign` шлёт `request_id`+`nanny_entity_id`+stable `idempotency_key`; истинный результат.
...
**Порядок в транзакции (важен — фикс Codex P1 #1: idempotency ПЕРЕД pair):**
```
BEGIN
1. SELECT user_id, (payload->>'status') AS status FROM parents WHERE id=$1 FOR UPDATE
   SELECT user_id FROM nannies WHERE id=$1 FOR UPDATE          → 404 если нет
2. parent_id/nanny_id NULL → 422
3. eligibility: status ∉ {NULL,'new','in_review'} → 409 (C2)
4. deletion-guard: любая строка account_deletions для [parent_id,nanny_id] → 409 (C4)
5. IDEMPOTENCY-FIRST: SELECT * FROM bookings WHERE idempotency_key=$1
     нашли + fp совпал → COMMIT? нет → ROLLBACK, 200 replay
...
const baseBody = {
  ...
  idempotency_key: 'bk:v1:deadbeef', date: '2026-07-10',
};
...
function computeFingerprint(input: {
  ...
  const date = new Date(input.date).toISOString().slice(0, 10); // UTC-дата
  // amount: TEXT (numeric-типизация — План E). Пустой/absent → сентинел \0.
  const amount = input.amount == null || input.amount.trim() === '' ? '\0' : input.amount.trim();
...
async function createBooking(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 1. lock+load parent(+status) и nanny (сериализует create пары под FOR UPDATE)
    ...
    // 3. eligibility (C2): NULL или new/in_review
    ...
    // 4. deletion-guard (C4): любая строка на любую сторону (в т.ч. state='deleted')
    ...
    // 5. IDEMPOTENCY-FIRST (Codex P1 #1): проверить ключ ДО pair-cardinality
    const existing = await client.query(
      'SELECT * FROM bookings WHERE idempotency_key = $1', [idempotency_key]);
    if (existing.rowCount && existing.rowCount > 0) {
      const row = existing.rows[0];
      await client.query('ROLLBACK'); // read-only ветка, транзакция не нужна
...
git commit -m "feat(bli141): create booking endpoint — provenance, eligibility, deletion-guard, idempotency-first, pair-cardinality"
```
...
function fpOf(b: { request_id: string; nanny_entity_id: string; date: string; amount?: string }) {
  ...
  const amount = b.amount == null || b.amount.trim() === '' ? '\0' : b.amount.trim();
  const canon = [b.request_id, b.nanny_entity_id, date, amount].join('\x1f');
  return 'sha256:v1:' + createHash('sha256').update(canon, 'utf8').digest('hex');
}
...
**Interfaces:**
- Produces: `POST /api/bookings?op=status` тело `{ booking_id, to_status }`. verifyBearerUser (участник) ИЛИ verifyBearerAdmin. Optimistic `UPDATE … WHERE id=? AND status=ANY(from)`. Ответы `200 {booking}` / `403` (actor не вправе) / `409` (stale/невалидный переход) / `404` / `400` / `401`.
...
describe('POST /api/bookings?op=status', () => {
  ...
  it('nanny moves confirmed → active (200)', async () => {
    ...
    const res = createMockResponse();
    await handler(statusReq({ booking_id: 'b1', to_status: 'active' }), res);
    expect(res.statusCode).toBe(200);
  });
  ...
  it('stale from-status → 409', async () => {
    ...
    connect.mockResolvedValue(mockClient({
      ...
      'UPDATE bookings SET status': { rows: [], rowCount: 0 }, // from не совпал
      BEGIN: { rows: [] }, ROLLBACK: { rows: [] },
    }));
    const res = createMockResponse();
    await handler(statusReq({ booking_id: 'b1', to_status: 'active' }), res);
...
**Files:**
- Create: `src/services/bookingKey.ts`
- Modify: `src/services/booking.ts` (create-path), `src/services/booking.test.ts`

**Interfaces:**
- Produces: `computeBookingKey(request_id, nanny_entity_id, date): string` (C3, детерминирован).
- Produces: `createBooking({ request_id, nanny_entity_id, date, amount? })` — authed POST `/api/bookings?op=create`; возвращает `Booking` или **бросает** (нет ложного local success). idempotency_key = `computeBookingKey(...)`.

- [ ] **Step 1: bookingKey helper + failing test**

```ts
// src/services/bookingKey.ts
// Детерминированный idempotency-ключ (C3): один интент (request_id, nanny_entity_id, date) → один ключ.
// WebCrypto (браузер/Capacitor). Ретрай той же кнопки даёт тот же ключ → сервер вернёт 200 replay.
export async function computeBookingKey(
  request_id: string, nanny_entity_id: string, date: string,
): Promise<string> {
  const canon = [request_id, nanny_entity_id, date].join('\x1f');
...
```
```ts
// src/services/bookingKey.test.ts
import { describe, it, expect } from 'vitest';
import { computeBookingKey } from './bookingKey';
describe('computeBookingKey', () => {
  it('стабилен на один интент', async () => {
    const a = await computeBookingKey('r1', 'n1', '2026-07-10');
    const b = await computeBookingKey('r1', 'n1', '2026-07-10');
    expect(a).toBe(b);
    expect(a.startsWith('bk:v1:')).toBe(true);
  });
  it('различается по нянe', async () => {
    expect(await computeBookingKey('r1', 'n1', '2026-07-10'))
      .not.toBe(await computeBookingKey('r1', 'n2', '2026-07-10'));
  });
});
```

- [ ] **Step 2: booking.test.ts — createBooking через endpoint**

Мок `supabase.auth.getSession()` → `{ data: { session: { access_token: 'tok' } } }` и глобальный `fetch`. Проверить: (a) POST на `/api/bookings?op=create` с `Authorization: Bearer tok` и телом `{request_id,nanny_entity_id,idempotency_key,date}`; (b) 201 → возвращает `body.booking`; (c) non-2xx → бросает, `trackBookingCreated` НЕ вызывается. Согласовать URL-базу с существующим `adminApi.ts` (тот же helper построения authed-запроса, если есть — переиспользовать).

- [ ] **Step 3: Run to verify fail** — `npm test -- src/services/booking.test.ts src/services/bookingKey.test.ts` → FAIL.

- [ ] **Step 4: Rewrite createBooking**

Заменить тело `createBooking` (booking.ts:124-155):
- убрать `remoteSaveBooking`/`markPendingBooking`/local upsert из create-path;
- сигнатура: `{ request_id, nanny_entity_id, date, amount? }` (без `parent_id`/`nanny_id`);
- `idempotency_key = await computeBookingKey(request_id, nanny_entity_id, date)`;
...
- [ ] **Step 5: Run to verify pass** — `npm test -- src/services/booking.test.ts src/services/bookingKey.test.ts && npm run build` → PASS (сигнатура обновлена у всех вызывающих — Task 6).

- [ ] **Step 6: Commit**

```bash
git add src/services/bookingKey.ts src/services/bookingKey.test.ts src/services/booking.ts src/services/booking.test.ts
git commit -m "feat(bli141): client createBooking -> server endpoint + stable idempotency key (без local-first write)"
```
...
- [ ] **Step 1: Failing test** — при назначении `createBooking` вызывается с `request_id = selectedParent.id`, `nanny_entity_id = nanny.id`, `date`, БЕЗ `parent_id`/`nanny_id`/`idempotency_key` (ключ считает сервисный слой из интента). При ошибке — `reportError`, без ложного success.
...
```
Убрать fallback `?? id` полностью. `reportSuccess` только после await; `catch` → `reportError` (есть). Ключ детерминирован в сервисе (C3): повтор той же кнопки в тот же день → тот же ключ → сервер 200 replay (без дубля). Разные дни — новый интент (ожидаемо).
...
`describe.skipIf(!process.env.INTEGRATION_PG_URL)`. `beforeAll`: подключить `pg.Pool` к `INTEGRATION_PG_URL`; в транзакции создать fixture — `parents(id,payload,user_id)` с `payload='{"status":"new"}'`, `nannies(id,payload,user_id)`. Вызывать `handler` напрямую (замокав только `_auth` на admin, `_cors`/`_rate-limit` пропускающими; **`_db` НЕ мокать** — реальный pool). `afterAll`: очистить fixture (DELETE по id/uid).
...
4. вторая пара при активной первой → 409.
5. parent `payload.status='rejected'` → 409 (eligibility на реальном jsonb).
6. параллельно два create одной пары (Promise.all) → ровно один 201, второй 409 (FOR UPDATE-сериализация).
...
- `api/bookings.ts`: create (провенанс + eligibility + deletion-guard + **idempotency-first** + pair-cardinality + 23505-reread), status (actor state-machine, optimistic), admin GET. Actor-scoped rate-limit (C5).
- Клиентский `createBooking`/`handleAssign` шлют entity-id + **стабильный** idempotency-key (из интента); сервер выводит auth-uid; нет `?? id` fallback; нет ложного local success.
- Прямой create-write в PostgREST удалён из `createBooking`.
- Unit + PG-backed integration тесты; точные коды на каждую ветку.

## Deploy gating (C0 — критично)

- **План B НЕ включается в проде сам по себе.** Прод-cutover = один бандл (§10): endpoint + удаление клиентских writer'ов + Плана E lockdown (RLS-split `bookings`, REVOKE write у `authenticated`, `bookings_active_pair_uq`) + lifecycle C (`account_deletions` наполнена). До бандла — только preview/staging + фиче-флаг.
- Пока `bookings_participant` (ALL) + `GRANT ALL authenticated` живы, эндпойнт-инварианты обходятся прямым PostgREST-write; endpoint становится единственным путём только после E.

## Out of scope (следующие планы)

- План C: `updateBookingStatus` клиента → status endpoint; `delete-account` lifecycle + `account_deletions` reconciler (наполняет таблицу → включает deletion-guard); удаление local **readers**/pending-merge (§7).
