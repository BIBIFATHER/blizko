...
**Revision:** rev4 — folded scoped Codex round 2 findings (exact Auth 404 contract,
ten-table RLS catalog proof, atomic migration/rollback, strict cleanup, stale-worker counters).
...
- **PC7 — Auth-delete 404-семантика:** успехом считается ТОЛЬКО подтверждённый `user_not_found`: HTTP 404 + валидный JSON-object + точное равенство одного из известных полей `error_code|code === 'user_not_found'` (Codex round2 P1). Substring-поиск запрещён: HTML/прокси-текст с упоминанием строки не доказывает ответ Auth. Любой другой 404 (роутинг, реверс-прокси, неверный URL, невалидный JSON) — ошибка, ретрай через attempts. Одинаковый parser-контракт действует в sync (Task 4) и reconciler (Task 5).
...
- `scripts/sql/rollback_20260702210000.sql` — атомарный rollback десяти barrier-политик + helper (Task 7; Codex round2 P1).
...
describe('DELETE /api/auth/delete-account', () => {
  ...
  it('Auth-delete 404 with user_not_found substring outside exact code -> 202 pending (PC7)', async () => {
    connect.mockResolvedValue(
      mockClient({
        'FROM parents': { rows: [{ user_id: UID }], rowCount: 1 },
        'FROM nannies': { rows: [], rowCount: 0 },
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (delResp.ok) {
      ...
      // PC7: только exact JSON error-code, не substring в произвольном теле.
      delOk = isConfirmedUserNotFound(delResp.status, bodyText);
...
describe('reconcile-account-deletions cron', () => {
  ...
  it('404 substring outside exact code -> stays db_done, lease released, attempts+1 (PC7)', async () => {
    query.mockImplementation(async (sql: string) => {
      if (sql.includes('SELECT') && sql.includes('FOR UPDATE SKIP LOCKED')) {
  ...
  it('stale worker completion rowCount=0 does not inflate response counters (Codex round2 P2)', async () => {
    query.mockImplementation(async (sql: string) => {
      if (sql.includes('SELECT') && sql.includes('FOR UPDATE SKIP LOCKED')) {
        return { rows: [{ user_id: 'u1', attempts: 0 }], rowCount: 1 };
      }
      return { rows: [], rowCount: 0 }; // newer worker already changed state
    });
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: true, status: 200, text: async () => '' })));
    const res = createMockResponse();
    await handler(makeReq(), res);
    ...
    try {
      ...
      const { rows } = await pool.query(
        `SELECT
           ...
           (SELECT count(*)::int FROM account_deletions WHERE user_id = ANY($3::uuid[])) +
...
```ts
  ...
  it('deletion write barrier catalog: exact restrictive contract exists on all 10 writer tables', async () => {
    ...
    const { rows } = await pool.query(
      `SELECT c.relname AS table_name, c.relrowsecurity, p.permissive, p.roles, p.cmd,
              p.qual, p.with_check
       FROM pg_class c
    ...
    expect(rows.map((r) => r.table_name)).toEqual([...expectedTables].sort());
...
-- Exact postconditions (Codex round2 P1): migration cannot report success with
-- one missing/non-restrictive policy, disabled RLS, or a weakened helper.
DO $$
DECLARE
...
BEGIN
  ...
  IF NOT has_function_privilege('authenticated', fn_oid, 'EXECUTE') OR EXISTS (
    SELECT 1
    FROM pg_proc p,
         LATERAL aclexplode(COALESCE(p.proacl, acldefault('f', p.proowner))) acl
    WHERE p.oid = fn_oid AND acl.grantee = 0 AND acl.privilege_type = 'EXECUTE'
  ) THEN
  ...
  IF EXISTS (
    SELECT 1
    FROM unnest(expected_tables) t(table_name)
...
Run: `psql "$INTEGRATION_PG_URL" -v ON_ERROR_STOP=1 -f supabase/migrations/20260702210000_bli139_deletion_write_barrier.sql`
Затем: `INTEGRATION_PG_URL=… npx vitest run api/delete-account.integration.test.ts`
Expected: PASS (все тесты файла, включая role-correct behavior + catalog contract для всех 10 таблиц). Прод НЕ трогаем.

- [ ] **Step 6: Write + rehearse rollback (Codex round2 P1)**

```sql
-- scripts/sql/rollback_20260702210000.sql
BEGIN;
DO $$
DECLARE t text;
BEGIN
...
BEGIN
  IF to_regprocedure('public.account_in_deletion()') IS NOT NULL OR EXISTS (
    SELECT 1 FROM pg_policy WHERE polname = ANY(expected_policies)
  ) THEN
...
Local rehearsal: apply → integration PASS → rollback with `ON_ERROR_STOP=1` →
verify helper + ten policies absent → re-apply forward migration → integration PASS.
The final local state remains forward-applied. Production remains untouched.

- [ ] **Step 7: Commit**

```bash
git add supabase/migrations/20260702210000_bli139_deletion_write_barrier.sql \
  scripts/sql/rollback_20260702210000.sql api/delete-account.integration.test.ts
git commit -m "feat(bli139): deletion write barrier — restrictive RLS + exact guards + rollback"
```
