...
- **Create deletion-guard (§3 шаг 3):** читает `account_deletions.state`; любое
  не-`deleted`-в-процессе (`deleting`/`db_done`) для стороны → `409`.
...
**Порядок шагов важен (Codex #4 #2): удалять unpaid-pending ДО массовой отмены,
иначе шаг отмены сделает их cancelled и шаг удаления их не найдёт.**

1. **Поставить deletion-флаг** под локом (durable `account_deletions.state =
   'deleting'`, см. §3.1) — закрывает гонку create-vs-delete (§3 шаг 3).
2. **Удалить незавершённые без оплаты** (status='pending' AND нет подтверждённого
...
3. **Отменить оставшиеся активные** брони уходящей стороны: `status → cancelled`
...
5. `COMMIT` БД-части → `account_deletions.state = 'db_done'`. **Затем** Auth-delete
