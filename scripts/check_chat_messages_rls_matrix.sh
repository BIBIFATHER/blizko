#!/usr/bin/env bash
# Runs the chat_messages RLS positive/negative matrix (scripts/sql/chat_messages_rls_matrix.sql).
# Self-contained BEGIN..ROLLBACK — applies the fix, seeds fixtures, asserts every
# identity, rolls back. Safe against prod or any linked database (nothing persists).
set -euo pipefail

sql_file="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/scripts/sql/chat_messages_rls_matrix.sql"
...
if [[ -n "$DATABASE_URL" ]]; then
  ...
  echo "Usage: DATABASE_URL=postgresql://... bash scripts/check_chat_messages_rls_matrix.sh"
  echo "   or: link the repo with Supabase CLI (SUPABASE_ACCESS_TOKEN in CI)"
  exit 1
fi

echo "OK: chat_messages RLS matrix passed"
