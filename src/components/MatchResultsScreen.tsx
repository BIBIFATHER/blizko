...
import { markMatchFollowUpProfileOpened, saveMatchFollowUp } from '@/services/matchFollowUp';
...
const CandidateCard: React.FC<{
  ...
  const copyShareLink = useCallback(() => {
    ...
    if (navigator.share) {
      navigator
        .share({ title: `Blizko: ${nanny.name}`, text: shareText, url: window.location.origin })
        .catch(() => {});
    } else {
            ...
            <div className="flex items-start gap-5">
              ...
              >
                {nanny.photo ? (
                  ...
                  <span className="text-2xl font-black text-white/90 drop-shadow-sm">
                    {initials}
                  </span>
                )}
              </motion.div>
            ...
            <motion.div
              ...
              transition={
                prefersReducedMotion
                  ? { duration: 0 }
                  : { ...layoutSpring, opacity: { duration: 0.2 } }
              }
            ...
            >
              <div className="space-y-4 px-6 pb-6 sm:px-8 sm:pb-8">
                ...
                <div className="rounded-2xl px-5 py-5" style={{ backgroundColor: '#F9F6F2' }}>
                  ...
                  <p className="text-[14px] leading-7 text-[#1C2B2D]">{humanExplanation}</p>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {trustBadges.length > 0 ? (
                      ...
                      <span className="text-[11px] font-medium italic text-stone-400">
                        {lang === 'ru'
                          ? 'Базовая проверка уже пройдена'
                          : 'Basic review already passed'}
                      </span>
                    )}
                  </div>
                </div>
...
export const MatchResultsScreen: React.FC<MatchResultsScreenProps> = ({ lang }) => {
  ...
  useEffect(() => {
    if (matchResult && matchResult.candidates.length > 0) {
      trackMatchingResults(matchResult.candidates.length, matchResult.candidates[0]?.score || 0);
    ...
    return (
      <div className="page-frame animate-fade-in py-4 pb-14 md:py-8">
        <h1 className="sr-only">
          {lang === 'ru' ? 'Подбираем няню для вашей семьи' : 'Finding a nanny for your family'}
        </h1>
  ...
  if (error) {
    return (
      <div className="page-frame animate-fade-in py-6">
        <h1 className="sr-only">
          {lang === 'ru' ? 'Результаты подбора няни' : 'Nanny match results'}
        </h1>
        <button
          onClick={() => navigate('/')}
          ...
          <div className="hero-grid">
            <div className="relative z-10 space-y-5">
              ...
              {matchResult.overallAdvice && (
                <div
                  className="rounded-[24px] border border-[color:var(--cloud-border)] bg-white/74 px-4 py-4 text-sm leading-7 text-stone-600 shadow-cloud-soft animate-pop-in"
                ...
                {[
                  {
                    ...
                    body:
                      lang === 'ru'
                        ? 'Только те, с кем есть смысл познакомиться.'
                        : 'Only profiles worth opening and discussing.',
                  },
                  {
                    title: lang === 'ru' ? 'Сигналы доверия видны сразу' : 'Trust appears early',
                    body:
                      lang === 'ru'
                        ? 'Проверки, отзывы и риски вынесены вверх карточки.'
                        : 'Checks, reviews, and risks are visible before deeper reading.',
                  },
                  {
                    title: lang === 'ru' ? 'Решение без спешки' : 'Decision without rush',
                    body:
                      lang === 'ru'
                        ? 'Можно открыть профиль, обсудить и вернуться к списку.'
                        : 'Open a profile, discuss it, and come back later.',
                  },
                ].map((item) => (
                  <div
                    key={item.title}
                    className="rounded-[1.6rem] bg-white/78 px-4 py-4 shadow-cloud-soft"
                  >
            ...
            <div className="relative z-10 grid gap-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div
                    key={item}
                    className="secondary-card px-3 py-3 text-center text-[11px] font-semibold text-stone-600 sm:text-xs"
                  >
                    {item}
                  </div>
                ))}
            ...
            <div className="space-y-6">
              {matchResult.candidates.slice(0, 3).map((candidate, index) => (
                <CandidateCard
                  ...
                  onToggle={() =>
                    setExpandedId(expandedId === candidate.nanny.id ? null : candidate.nanny.id)
                  }
