...
export const ParentSummaryModal: React.FC<Props> = ({
  lang,
  formData,
  advanced,
  onConfirm,
  ...
  const ageLine = formData.childAge
    ? ru
      ? `Ребёнок: ${formData.childAge}`
      : `Child: ${formData.childAge}`
    : null;
  const scheduleLine = formData.schedule
    ? ru
      ? `График: ${formData.schedule}`
  ...
  if (advanced.cameras && advanced.cameras !== 'ok')
    conditionTags.push(ru ? 'Без камер' : 'No cameras');
  if (advanced.household)
    conditionTags.push(
      ru ? `Помощь по дому: ${advanced.household}` : `Household: ${advanced.household}`,
    );
  if (advanced.pets)
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-3">
        ...
        <div className="rounded-2xl bg-white/70 p-4 border border-[#1C2B2D]/6">
          ...
          <div className="space-y-1.5">
            {ageLine && <p className="text-sm text-[#1C2B2D]/80">{ageLine}</p>}
            {scheduleLine && <p className="text-sm text-[#1C2B2D]/80">{scheduleLine}</p>}
            {storySnippet && (
              <p className="text-sm leading-relaxed text-[#1C2B2D]/65 italic">
                «{storySnippet}
                {formData.comment.trim().length > 120 ? '…' : ''}»
              </p>
            )}
            ...
            {conditionTags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {conditionTags.map((tag, i) => (
                  <span
                    key={i}
                    className="rounded-full bg-stone-100 px-2.5 py-0.5 text-xs text-[#1C2B2D]/60"
                  >
                    {tag}
                  </span>
                ))}
