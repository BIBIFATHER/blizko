...
function formatActionLabel(entry: AdminActionEntry): string {
  ...
  if (entry.action === 'bulk_verify_profiles')
    return `Массовое подтверждение профилей${entry.meta?.count ? `: ${entry.meta.count}` : ''}`;
  if (entry.action === 'bulk_docs_status')
    return `Массовое обновление документов${entry.meta?.status ? `: ${entry.meta.status}` : ''}${entry.meta?.count ? ` • ${entry.meta.count}` : ''}`;
          ...
          {filtered.map((entry, i) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <div
                  className={`text-sm font-medium ${entry.kind === 'error' ? 'text-red-700' : entry.kind === 'success' ? 'text-green-700' : 'text-stone-700'}`}
                >
                  {formatActionLabel(entry)}
                </div>
              ...
              <div className="shrink-0 flex flex-col items-end gap-1">
                ...
                {entry.adminId && (
                  <div
                    className="text-[10px] font-mono text-stone-400 bg-stone-100/80 rounded px-1.5 py-0.5 whitespace-nowrap"
                    title={entry.adminId}
                  >
