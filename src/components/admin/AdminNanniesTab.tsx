      ...
      <section>
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <AdminPillButton onClick={() => exportNanniesOpsCsv(filteredNannies)} tone="warm">
              Выгрузить для Ops
            </AdminPillButton>
        ...
        <div className={`${adminSectionPanel} mb-3`}>
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            <AdminPillButton onClick={bulkVerifyVisible} tone="success">
            ...
            <AdminPillButton onClick={() => bulkSetDocsStatusVisible('pending')} tone="warm">
          ...
          <div className="space-y-3">
            {filteredNannies.map((n) => {
              ...
              const isProb =
                ...
                docs.some(
                  (d) =>
                    d.status === 'rejected' || d.status === 'pending' || (d.aiConfidence || 0) < 70,
                ) ||
              ...
              return (
                <Card key={n.id} className="p-4!">
                  <div className="flex justify-between text-xs text-stone-400 mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      {isProb && <Badge variant="status">проблема</Badge>}
                  ...
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    {(n.video || n.videoIntro) && (
                      <AdminPillButton
                        ...
                        disabled={!n.video}
                        title={
                          n.video ? 'Открыть видео-интро' : 'Видео загружено, ссылка не сохранена'
                        }
                      >
                        <PlayCircle size={14} />
                  ...
                  {n.softSkills && (
                    <div className={`mt-3 ${adminSubsectionPanel}`}>
                      <div className="flex items-center gap-1 text-xs font-bold text-amber-600 mb-1">
                        <BrainCircuit size={12} /> Поведенческий профиль (
                      ...
                      <div className="space-y-2">
                        {n.reviews.map((r: Review, idx: number) => (
                          <div key={idx} className={adminSubsectionPanel}>
                            ...
                            {r.text && (
                              <div className="text-[10px] text-stone-600 mt-1">{r.text}</div>
                            )}
                          </div>
                        ))}
                      </div>
