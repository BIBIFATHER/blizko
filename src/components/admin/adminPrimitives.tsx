...
const adminPillToneClasses: Record<AdminPillTone, string> = {
  neutral:
    'bg-white/70 text-stone-700 border-stone-200/80 hover:bg-white hover:border-amber-200/60',
...
export const adminModalSurface = 'surface-panel rounded-[2rem] shadow-2xl overflow-hidden';

export const adminModalHeader = 'hero-shell p-4 sm:p-5 flex items-start justify-between gap-4';

interface AdminPillButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  active?: boolean;
  tone?: AdminPillTone;
}
...
export const AdminPillButton: React.FC<AdminPillButtonProps> = ({
  ...
  return (
    <button className={`${adminPillButtonBase} ${toneClass} ${className}`} {...props}>
      {children}
    </button>
  );
};
