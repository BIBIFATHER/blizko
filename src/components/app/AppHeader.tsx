...
export function AppHeader({
  ...
  const screenTitle = (() => {
    ...
    if (pathname === '/match-results')
      return lang === 'ru' ? 'Подборка от куратора' : 'Your options';
  ...
  const screenHint = (() => {
    ...
    if (pathname === '/match-results')
      return lang === 'ru' ? 'Подобранные профили' : 'Curated profiles';
    if (pathname.startsWith('/nanny/'))
      return lang === 'ru' ? 'Проверка и контекст' : 'Trust and context';
    if (pathname === '/find-nanny')
      return lang === 'ru' ? 'Заполняем шаг за шагом' : 'Step by step';
    return lang === 'ru' ? 'Спокойный выбор' : 'Calm decision';
  })();

  return (
    <div
      className={`sticky top-0 z-30 transition-all duration-300 pt-safe ${isScrolled ? 'pb-2.5' : 'pb-3.5'}`}
    >
      <div className="page-frame">
        <div
          className={`floating-bar app-topbar flex items-center justify-between rounded-[26px] px-3 py-2.5 md:px-5 ${isScrolled ? 'translate-y-0 opacity-100' : 'bg-white/62'}`}
        >
          ...
          <div className="flex min-w-0 items-center gap-2 pr-safe">
            ...
            <button
              ...
              aria-label={
                lang === 'ru' ? 'Переключить язык на английский' : 'Switch language to Russian'
              }
            >
              {lang === 'ru' ? 'EN' : 'RU'}
            </button>
            ...
            {!user ? (
              ...
              <div className="flex items-center gap-2">
                <span
                  className="hidden lg:inline text-[11px] text-stone-500 bg-white/86 border border-stone-200 rounded-full px-2.5 py-1"
