...
export interface NannyFormContextType {
  ...
  setToast: React.Dispatch<
    React.SetStateAction<{ message: string; type: 'error' | 'info' } | null>
  >;
...
export const NannyFormProvider: React.FC<{
  children: ReactNode;
  initialData?: NannyProfile;
  lang?: Language;
}> = ({ children, initialData, lang: _lang }) => {
  const totalSteps = 4;
  ...
  const [softSkills, setSoftSkills] = useState<SoftSkillsProfile | undefined>(
    initialData?.softSkills,
  );
  const [documents, setDocuments] = useState<DocumentVerification[]>(initialData?.documents || []);
  const [resumeNormalized, setResumeNormalized] = useState<NormalizedResume | undefined>(
    initialData?.resumeNormalized,
  );
    ...
    if (result.success) {
      ...
      if (!value) {
        setToast({
          message:
            lang === 'ru'
              ? 'Не удалось определить город/район автоматически'
              : 'Could not detect city/district automatically',
          type: 'info',
        });
      }
