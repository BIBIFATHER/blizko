    ...
    <div className="animate-fade-in space-y-6 relative pb-32">
      ...
      <div className="flex justify-center mb-6">
        <label className="relative cursor-pointer group flex flex-col items-center">
          <div
            className={`w-32 h-32 rounded-full overflow-hidden flex items-center justify-center transition-all shadow-cloud-soft bg-white/40 backdrop-blur-sm border-2 ${photo ? 'border-amber-300' : 'border-amber-100/50'}`}
          >
            {photo ? (
              ...
              <div className="flex flex-col items-center gap-1.5">
                ...
                />
                <span className="text-[11px] font-semibold text-amber-700/60 uppercase tracking-tighter">
          ...
          <div className="flex justify-center items-center gap-1.5 text-[10px] text-[#2A6B6E] font-semibold bg-[#EFF3F2] px-3 py-1 rounded-full mx-auto w-fit">
            <ShieldCheck size={12} />{' '}
            {lang === 'ru' ? 'Данные защищены и не передаются' : 'Data protected, never shared'}
          </div>
        </label>
      </div>
      ...
      <div className="relative">
        ...
        <div className="mt-3 flex items-start gap-2 text-[11px] text-stone-500 leading-tight bg-stone-50 p-2.5 rounded-xl border border-stone-100">
          ...
          <span>
            {lang === 'ru'
              ? 'Ваши контакты скрыты. Семья увидит их только после взаимной симпатии.'
              : 'Your contacts are hidden. Family will see them only after a mutual match.'}
          </span>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          <div className="mt-1">
            <Checkbox
              label={
                lang === 'ru'
                  ? 'Готова работать с двумя семьями поблизости'
                  : 'Ready to work with two nearby families'
              }
          ...
          <div className="w-full bg-[#EFF3F2] shadow-cloud-soft border border-[#7FA99B]/40 rounded-2xl p-3 animate-slide-up flex items-center gap-3 mb-4">
            ...
            <div className="text-[11px] text-[#1C2B2D] leading-tight">
              {lang === 'ru' ? (
                ...
                <span>
                  Great! Keep going — families in your area may already be looking for someone just
                  like you.
                </span>
              )}
            </div>
