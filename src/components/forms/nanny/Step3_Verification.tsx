...
export const Step3_Verification: React.FC<Props> = ({ lang }) => {
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-32">
      ...
      <Card
        className={`transition-all duration-300 ${isVerified ? 'bg-[#EFF3F2] border-[#7FA99B]/40 shadow-sm' : 'bg-white/60 backdrop-blur-md border border-white/60 shadow-sm'}`}
      >
        <div className="flex items-start gap-4">
          <div
            className={`p-3 rounded-full shrink-0 ${isVerified ? 'bg-[#2A6B6E]/15 text-[#2A6B6E]' : 'bg-stone-100 text-stone-400'}`}
          >
            <ShieldCheck size={24} />
          </div>
      ...
      <Card
        className={`transition-all duration-300 ${documents.length > 0 ? 'bg-[#EFF3F2] border-[#7FA99B]/40 shadow-sm' : 'bg-white/60 backdrop-blur-md border border-white/60 shadow-sm'}`}
      >
        <div className="flex items-start gap-4">
          <div
            className={`p-3 rounded-full shrink-0 ${documents.length > 0 ? 'bg-[#2A6B6E]/15 text-[#2A6B6E]' : 'bg-stone-100 text-stone-400'}`}
          >
            <FileText size={24} />
          </div>
          <div className="flex-1">
            <h3 className="font-semibold text-stone-800 mb-1">{text.docsTitle}</h3>
            ...
            {documents.length > 0 && (
              <div className="animate-fade-in space-y-2">
                <div className="flex items-center gap-2 text-sm text-[#2A6B6E] font-medium mb-2">
                  <Check size={16} /> {documents.length}{' '}
                  {lang === 'ru' ? 'документов загружено' : 'documents uploaded'}
                </div>
                {documents.map((doc, i) => (
                  <div
                    key={i}
                    className="bg-white/60 p-2 rounded-lg flex justify-between items-center border border-[#7FA99B]/30"
                  >
                ...
                {resumeNormalized && (
                  <div className="bg-white border border-[#7FA99B]/30 rounded-lg p-2 text-xs text-stone-600">
                    <div className="font-semibold text-stone-700 mb-1">
                      Резюме распознано в едином формате
                    </div>
                    <div>Имя: {resumeNormalized.fullName || '—'}</div>
                    <div>Город: {resumeNormalized.city || '—'}</div>
                    <div>
                      Опыт:{' '}
                      {typeof resumeNormalized.experienceYears === 'number'
                        ? `${resumeNormalized.experienceYears} лет`
                        : '—'}
                    </div>
                  </div>
                )}
      ...
      <Card
        className={`transition-all duration-300 ${readinessSnapshot.qualityApproved ? 'bg-[#EFF3F2] border-[#7FA99B]/40 shadow-sm' : 'bg-[#F9F6F2] border-[#7FA99B]/30 shadow-sm'}`}
      >
        <div className="flex items-start gap-4">
          <div
            className={`p-3 rounded-full shrink-0 ${readinessSnapshot.qualityApproved ? 'bg-[#2A6B6E]/15 text-[#2A6B6E]' : 'bg-[#C4744A]/15 text-[#C4744A]'}`}
          >
            <Check size={24} />
          </div>
          <div className="flex-1">
            ...
            {readinessSnapshot.qualityApproved ? (
              ...
              <div className="space-y-2">
                <div className="text-xs font-semibold text-stone-600">
                  {lang === 'ru'
                    ? 'Чтобы профиль увидели семьи:'
                    : 'For families to see your profile:'}
                </div>
