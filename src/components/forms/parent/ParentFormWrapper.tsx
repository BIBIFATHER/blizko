  ...
  const submitData = async () => {
    ...
    try {
      ...
      if (formData.dateFrom || formData.dateTo || slots) {
        structured.push(
          `Период: ${formData.dateFrom || '—'} → ${formData.dateTo || '—'}${slots ? ` | Слоты: ${slots}` : ''}`,
        );
      }
      ...
      if (formData.extraPhrases.length > 0)
        storyParts.push(`Пожелания: ${formData.extraPhrases.join(', ')}`);
      const analysisBlock =
        storyParts.length > 0 ? `\n\n[Для анализа]\n${storyParts.join('\n')}` : '';
  ...
  return (
    <>
        ...
        lockedNotice={
          initialDataStatus === 'approved' && isEditing ? (
            <div className="bg-amber-50/60 text-amber-800 p-3 rounded-2xl text-sm font-medium mb-4 flex items-center justify-center border border-amber-200/40">
              {lang === 'ru'
                ? 'Заявка одобрена. Редактирование заблокировано.'
                : 'Request is approved. Editing is locked.'}
            </div>
          ) : undefined
        }
        onBack={onBack}
      ...
      >
        ...
        {currentStep === 3 && (
          <Step3_FamilyProfile lang={lang} onFinalSubmit={handleFinalSubmit} loading={loading} />
        )}
      </StepWizardShell>
      ...
      {showOffer && (
        <ParentOfferModal onClose={() => setShowOffer(false)} onAccept={submitData} lang={lang} />
      )}
    </>
  );
};
