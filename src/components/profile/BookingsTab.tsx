...
import { Calendar, MessageSquare, CheckCircle, Star, MapPin, Briefcase } from 'lucide-react';
...
import {
  getBookingsForUser,
  updateBookingStatus,
  type Booking as ServiceBooking,
...
export const BookingsTab: React.FC<BookingsTabProps> = ({ user, lang, onReviewSubmit }) => {
  ...
  const getAvatarColor = (seed: string) => {
    ...
    const index =
      seed.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0) % palette.length;
  ...
  const activeBooking = useMemo(
    () =>
      upcomingBookings.find((booking) => booking.status === 'active') ||
      upcomingBookings[0] ||
      null,
    [upcomingBookings],
  );
              ...
              <Card key={booking.id} className="p-5! border-stone-200">
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <div className="text-[11px] text-stone-500 bg-stone-50 border border-stone-100 rounded-lg p-2 space-y-1">
                    <div>
                      {lang === 'ru'
                        ? 'Показываем только реальные бронирования из системы.'
                        : 'Only real bookings from the system are shown here.'}
                    </div>
                    <div>
                      {lang === 'ru'
                        ? 'Если статус изменился на другом устройстве, он подтянется при следующей синхронизации.'
                        : 'If the status changes on another device, it will sync on the next refresh.'}
                    </div>
                  </div>
                </div>
              </Card>
                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${getAvatarColor(activeBooking.id)}`}
                        >
                          {getCounterpartyName(activeBooking).charAt(0)}
                        </div>
                        <div>
                          <div className="font-bold text-stone-800">
                            {getCounterpartyName(activeBooking)}
                          </div>
                          <Badge
                            variant={
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <div className="bg-white/70 border border-amber-200 rounded-lg p-2 text-[11px] text-amber-700">
                        <div className="font-semibold">
                          {lang === 'ru' ? 'Реальное бронирование' : 'Live booking'}
                        </div>
                        <div>
                          {lang === 'ru'
                            ? `ID заявки: ${activeBooking.request_id}`
                            : `Request ID: ${activeBooking.request_id}`}
                        </div>
                        <div>
                          {lang === 'ru'
                            ? 'Статус можно уточнить через чат или поддержку.'
                            : 'You can clarify status through chat or support.'}
                        </div>
                      </div>
                      <Button
                        onClick={() => setChatBooking(activeBooking)}
                        className="py-3 text-sm bg-stone-800 text-white hover:bg-stone-700 w-full"
                      >
            ...
            <div>
              ...
              <div className="space-y-3">
                {historyBookings.map((booking) => (
                  <Card
                    key={booking.id}
                    className="p-4! flex flex-col gap-2 border-stone-100 bg-white"
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${getAvatarColor(booking.id)}`}
                        >
                          {getCounterpartyName(booking).charAt(0)}
                        </div>
                        <div>
                          <div className="font-semibold text-stone-700 text-sm">
                            {getCounterpartyName(booking)}
                          </div>
