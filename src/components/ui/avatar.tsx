    ...
    <div className={`relative inline-flex shrink-0 ${className}`}>
      ...
      >
        {showImage ? (
          ...
          <div
            className={`flex h-full w-full items-center justify-center bg-amber-50 ${s.text} font-semibold text-amber-700`}
          >
            {getInitials(name)}
          </div>
        )}
      ...
      {badge && (
        <span
          className={`absolute ${s.badge} flex items-center justify-center rounded-full ${
            badge === 'verified' ? 'bg-green-500 text-white' : 'bg-amber-400 text-white'
          } ring-2 ring-white`}
