...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variants = {
    ...
    secondary: 'surface-panel cta-secondary border-white/70 text-stone-700 hover:bg-white/95',
    outline:
...
export const Checkbox: React.FC<CheckboxProps> = ({ label, checked, onChange }) => (
  <label className="ml-[-0.25rem] flex min-h-[44px] cursor-pointer select-none items-center gap-3 rounded-xl px-1 py-3 transition-all active:scale-[0.98]">
    ...
    >
      {checked && <Check className="animate-scale-in text-stone-700" size={14} strokeWidth={3} />}
    </div>
...
export const ChipGroup: React.FC<ChipGroupProps> = ({
  ...
  return (
    <div className="mb-5" ref={containerRef}>
      <label className="ml-1 mb-2.5 block text-sm font-medium text-stone-500">{label}</label>
