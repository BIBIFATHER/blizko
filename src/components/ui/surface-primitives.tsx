...
export const Section: React.FC<SectionProps> = ({ children, className = '', ...props }) => (
  <section className={`section-shell p-5 sm:p-6 ${className}`} {...props}>
    {children}
  </section>
);
...
export const CuratorsNote: React.FC<CuratorsNoteProps> = ({
  ...
  <div className={`relative rounded-2xl bg-amber-50/50 px-6 pt-8 pb-6 ${className}`}>
    ...
    {author && <p className="mt-3 text-xs font-medium text-stone-400">— {author}</p>}
  </div>
);
