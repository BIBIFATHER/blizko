...
function getMaxScore(request: MatchingRequest, w: MatchingWeights, requirementCount: number, needCount: number) {
  ...
  return dedupeCandidates(candidates)
    ...
    .map((nanny) => {
      ...
      if (nanny.softSkills?.rawScore) {
        const softSkillsScore = Math.min(w.softSkillsMax, Math.round(nanny.softSkills.rawScore / 20));
