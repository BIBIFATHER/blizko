...
function buildHumanExplanationFallback(
  ...
  if (nanny.district || nanny.metro) {
    ...
    if (location) parts.push(lang === "ru" ? `работает рядом (${location})` : `nearby (${location})`);
  }
  ...
  if (lang === "ru") {
    return validParts.length === 0 ? `${name} может подойти вашей семье.` : `${name} — ${validParts.join(", ")}.`;
  }

  return validParts.length === 0 ? `${name} could be a good match for your family.` : `${name} — ${validParts.join(", ")}.`;
}
...
export async function buildMatchResult(
  ...
  const explanations = await Promise.all(top3.map((candidate) => buildHumanExplanationAI(candidate, request, lang)));
