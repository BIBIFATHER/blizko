...
export function detectRiskFlags(
  ...
  if (rp.nannyStylePreference && np.disciplineStyle) {
    if (rp.nannyStylePreference === 'gentle' && np.disciplineStyle === 'strict') {
      flags.push({
        ...
        advice:
          'Обсудите конкретные ситуации: что делать при отказе есть, убирать игрушки, истерике',
      });
    }
  ...
  const isInfant =
    childAge.includes('0') ||
    childAge.includes('до 1') ||
    childAge.includes('грудн') ||
    childAge.includes('новорожд');
  if (
    isInfant &&
    !nannyAges.includes('0') &&
    !nannyAges.includes('грудн') &&
    !nannyAges.includes('до 1')
  ) {
    flags.push({
      level: 'critical',
  ...
  if (isInfant) {
    const hasMedBook = nanny.documents?.some(
      (d) => d.type === 'medical_book' && d.status === 'verified',
    );
    if (!hasMedBook) {
      flags.push({
        level: 'critical',
