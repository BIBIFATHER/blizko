          ...
          resolve({
            ...
            error: !(city || district)
              ? lang === 'ru'
                ...
                : 'Could not detect city automatically'
              : undefined,
          });
        } catch (err) {
      ...
      (err) => {
        ...
        switch (err.code) {
          case err.PERMISSION_DENIED:
            errorMsg =
              lang === 'ru'
                ? 'Доступ к геолокации запрещен. Проверьте настройки браузера или системы.'
                : 'Location access denied. Please check your browser or system settings.';
            break;
